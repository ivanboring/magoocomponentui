...
export async function buildFilesFor(id, target) {
  ...
  if (target === "html") {
    ...
    return { [`${def.name}.html`]: renderToHtml(ast, { ...(await defaultExample(dir)), $variants: meta.def.variants }) };
  }
  return filesForTarget(files, target);
}
...
export async function runBuild(argv) {
  ...
  for (const id of ids) {
    for (const [rel, contents] of Object.entries(await buildFilesFor(id, target))) {
